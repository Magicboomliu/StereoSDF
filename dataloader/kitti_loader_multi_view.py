...
class StereoDataset(Dataset):
    def __init__(self, 
        ...
        self.new_view_dir = new_view_dir
        ...
        for line in lines:
            ...
            if self.mode=='train':
                sample['left_left'] = sample['left'].replace(self.data_dir,self.new_view_dir)
                basename = os.path.basename(sample['left'])
                sample['left_left'] = sample['left_left'].replace(basename,"left_left_from_left_"+basename)


                sample['right_right'] = sample['right'].replace(self.data_dir,self.new_view_dir)
                basename = os.path.basename(sample['right'])
                sample['right_right'] = sample['right_right'].replace(basename,"right_right_from_right_"+basename)
    ...
    def __getitem__(self, index):
        ...
        if self.mode =='train':
            if "left_left" in sample_path.keys():
                sample['img_left_left'] = read_img(sample_path['left_left'])

            if "right_right" in sample_path.keys():
                sample['img_right_right'] = read_img(sample_path['right_right'])
