...
# Encoder Blocks With Confience.
class EncoderB_Conf(nn.Module):
    def __init__(self, n_blocks, channels_in, channels_out, downsample=False):
        super(EncoderB_Conf, self).__init__()
        body = []
        if downsample:
            body.append(nn.Sequential(
                nn.Conv2d(channels_in, channels_out, 3, 2, 1, bias=False),
        ...
        nn.Conv2d(in_channels=channels_out,out_channels=channels_out//2,kernel_size=3,stride=1,padding=1,bias=False),
        nn.BatchNorm2d(channels_out//2),
        nn.Conv2d(in_channels=channels_out//2,out_channels=channels_out//2,kernel_size=3,stride=1,padding=1,bias=False),
        nn.BatchNorm2d(channels_out//2),
        nn.Conv2d(in_channels=channels_out//2,out_channels=1,kernel_size=3,stride=1,padding=1,bias=False),
        nn.Sigmoid())

    def forward(self, x):
        feat = self.body(x)
        
        confidence = self.confidence_branch(feat)
        
        return feat,confidence
    

# Decoder Blocks With Confience.
class DecoderB_Conf(nn.Module):
    def __init__(self, n_blocks, channels_in, channels_out):
        super(DecoderB_Conf, self).__init__()
        body = []
        body.append(nn.Sequential(
                nn.Conv2d(channels_in, channels_out, 1, 1, 0, bias=False),
                nn.BatchNorm2d(channels_out),
        ...
        # confidnce branch.
        self.confidence_branch = nn.Sequential(
        nn.Conv2d(in_channels=channels_out,out_channels=channels_out//2,kernel_size=3,stride=1,padding=1),
        nn.BatchNorm2d(channels_out//2),
        nn.Conv2d(in_channels=channels_out//2,out_channels=channels_out//2,kernel_size=3,stride=1,padding=1),
        nn.BatchNorm2d(channels_out//2),
        nn.Conv2d(in_channels=channels_out//2,out_channels=1,kernel_size=3,stride=1,padding=1),
        nn.Sigmoid())

    def forward(self, x):
        
        feat = self.body(x)
        confidnce = self.confidence_branch(feat)
        
        return feat,confidnce
...
class PAB(nn.Module):
    ...
    def forward(self, x_left, x_right, cost):
        ...
        # left and right feature aggregation
        fea_left = self.head(x_left)
        fea_right = self.head(x_right)

        # left feature as the query feature, right feature as the key feature
        ...
        # right feature as the query feature, left feature as the key feature
        ...
        # update the left feature.
        # update the right feature.
        # update the leftright cost volume.
        # update the rightleft cost volume.
...
class Output(nn.Module):
    ...
    def forward(self, cost, max_disp):
        # cost_right2left is for disparity.
        # cost_left2right is for occlusion.
        ...
        #  right-top is zero, here what the author do is normalization.
        ...
        # valid mask (left image): non-occluded regions.
...
class OutputV2(nn.Module):
    ...
    def forward(self, cost, max_disp):
        ...
        # disparity
        disp,_ = regress_dispV2(att_right2left, valid_mask_left)
...
# Disparity Regression
def regress_dispV2(att, valid_mask):
    '''
    :param att:         B * H * W * W
    :param valid_mask:  B * 1 * H * W
    '''
    ...
    # valid_mask_0 = valid_mask
    disp = disp_ini
    # disp = disp_ini * valid_mask_0 # only Kept the non-occluded regions
